...
function Scene2() {
    ...
    let terrain = [];
    for (let i = 0; i < 10; i++) {
        terrain.push(new StaticImage(images.flower, 4, 200 + Math.random() * 400, -40 - Math.random() * 10))
    }

    let trees = [0, 1, 2].flatMap(b => [0, 1, 2].flatMap(a => [
        new StaticImage(images.tree, 3, 500 - a * 300 + b * 1000, -100),
        new StaticImage(images.tree, 3, 700 - a * 500 + b * 1000, -100),
        new StaticImage(images.tree, 2.5, 700 - a * 400 + b * 1000, -100),
        new StaticImage(images.tree, 2, 300 - a * 250 + b * 1000, -100)
    ]));
    ...
    let initialS2s = [
        ...terrain,
        new Rect(grassGradient, 0, -45, 1800, 52),
        new Rect("#0f5c23", 0, 50, 1800, 140),
        new Ground(0, 500),
        dobbs,
        dobbsname,
        ...coins,
    ...
    terrain.forEach(a => {
        a.Update = () => {
            ...
            a.x -= 1.5;
            if (a.tileset === images.tree) a.x -= a.scale / 4
...
function Scene6(panic, balloon, basket) {
    ...
    let comicPanel3 = new Rect("#111650", 250, 37, 148, 320);

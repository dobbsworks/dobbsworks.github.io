...
var Sprite = /** @class */ (function () {
    ...
    Sprite.prototype.ReactToSolids = function () {
        ...
        // Slopes
        var centerBottomTile = this.layer.GetTileByPixel(this.xMid, this.yBottom - 1);
        if (centerBottomTile) {
            var solidity = centerBottomTile.tileType.solidity;
            if (solidity instanceof SlopeSolidity && solidity.verticalSolidDirection == 1) {
                // is sloping floor
                if (solidity.GetIsPointInSolidSide(this.xMid, this.yBottom, this.layer, centerBottomTile)) {
                    var targetY = solidity.GetSlopePoint(this.xMid, this.layer, centerBottomTile);
                    ...
                    if (ceilingHeight > -1) {
                        ...
                        if (heightToScootUp > headRoom) {
                            ...
                            this.touchedCeilings.push(centerBottomTile);
                        }
                    }
                    if (this.rolls) {
        ...
        for (var _i = 0, pixelsToCheck_2 = pixelsToCheck; _i < pixelsToCheck_2.length; _i++) {
            ...
            var _loop_2 = function (colIndex) {
                ...
                if (!this_2.layer.tiles[colIndex])
                    return "continue";
                var tile = this_2.layer.tiles[colIndex][rowIndex];

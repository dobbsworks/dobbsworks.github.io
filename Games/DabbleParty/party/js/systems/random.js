...
var Random = /** @class */ (function () {
    ...
    Random.GetSeededRandInt = function (lowestInt, highestInt) {
    ...
    Random.SeededRandFrom = function (list) {
        if (list.length == 0) {
            throw "RandFrom called on empty list";
        }
        if (list.length == 1)
            return list[0];
        var index = Random.GetSeededRandInt(0, list.length - 1);
        return list[index];
    };
    ...
    Random.GetShuffledCopy = function (list) {
        ...
        for (var i = 0; i < list.length; i++) {
            var indexToRemove = Random.GetSeededRandInt(0, copy.length - 1);
